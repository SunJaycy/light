...
def _safe_unregister():
    """Unregister all classes/property if they were already loaded.

    Blender keeps previous registrations alive when the script is reloaded from
    the Text Editor, which can lead to duplicate UI panels or "register class"
    errors. This helper clears any existing registration before we register
    again, keeping the script reload-friendly.
    """
    ...
    for cls in reversed(classes):
        try:
            ...
            # It might not be registered yet; that's fine.
            pass

    if hasattr(bpy.types.Scene, "lightmap_settings"):
        try:
